...
| FAISS                | Biblioteca embarcada    | A estudar | `faiss_examples/`        |
...
| Milvus               | Distribuído / embarcado | A estudar | `milvus_examples/`       |
...
| FAISS               | Biblioteca local        | Pequena/grande | Performance máxima em processo único, pipelines de ML  |
...
| Milvus              | Self-hosted / cloud     | Muito grande   | Escala bilionária, arquitetura distribuída, produção   |
...
├── faiss_examples/            # Exemplos com FAISS (Meta AI)
├── milvus_examples/           # Exemplos com Milvus
...
3. FAISS             → biblioteca embarcada, performance máxima, benchmarking
4. Qdrant            → performance, filtros avançados, client REST/gRPC
5. Weaviate          → busca semântica avançada, schema, GraphQL
6. Milvus            → escala distribuída, múltiplos tipos de índice
7. Pinecone          → produção em nuvem, serverless, alta escala
8. Amazon S3 Vectors → armazenamento vetorial serverless na AWS, integração com Bedrock
9. Amazon OpenSearch → busca híbrida (vetorial + full-text), ecossistema AWS
```

## Pré-requisitos gerais

- Python 3.11+
- Docker (para pgvector, Qdrant, Weaviate, Milvus e OpenSearch)
